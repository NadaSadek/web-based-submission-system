...
<link rel="stylesheet" href="style.css">
	...
	$query = mysql_query("SELECT * FROM authors WHERE accepted=2 AND admin=0");
	$count=0;
...
$em=$_POST["email"];
$na=$_POST["nam"];
	$q = mysql_query("UPDATE authors SET accepted=1 WHERE email='$em'") or die(mysql_error());
	echo '<span style="color:green;text-align:center;">'.$na.' ('.$em.')'.' is now unblocked</span></br>';
}
 else {   $name=$row['Name'];     
	$value2=$row['email']; 
...
<input type="hidden" id="name" name="nam" value="'.$name.'">
 Name:'.$name.'</br>
 Email: '.$value2.' </br>
 <span class="ButtonInput"><span> <input type="submit" name="'.$count.'" value="unblock"></span></span>';
 echo'</form></div></div>';
 }
 }
echo '</body></html>';
